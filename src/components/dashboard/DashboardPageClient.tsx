...
export default function DashboardPageClient() {
  ...
  const dashboardDict = dict.dashboardPage;
  const projectsDict = dict.projectsPage;
  ...
  useEffect(() => {
    ...
    async function getDashboardData() {
      try {
        ...
        if (!projectsRes.ok || !leaveRes.ok || !settingsRes.ok || !usersRes.ok || !attendanceRes.ok) {
            throw new Error("Failed to fetch one or more resources for dashboard.");
        }
  ...
  const { eventsByDate, upcomingEvents } = useMemo(() => {
    ...
    data.leaveRequests.filter(l => l.status === 'Approved').forEach(l => {
  ...
  const attendanceSummary = useMemo(() => {
    ...
    data.leaveRequests.filter(l => l.status === 'Approved').forEach(l => {
      if (isWithinInterval(today, { start: startOfDay(parseISO(l.startDate)), end: endOfDay(parseISO(l.endDate)) })) {
